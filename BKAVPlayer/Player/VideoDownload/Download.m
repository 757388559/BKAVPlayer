...
@interface Download ()<NSURLConnectionDelegate , NSURLConnectionDataDelegate>
...
/// 缓存路径类.
@property (nonatomic , strong) BKPlayerCachePath *bkPlayerCachePath;
...
- (void)setUrl:(NSURL *)url offset:(NSUInteger)offset {
    ...
    if (self.taskArr.count >= 1) {
        [self.bkPlayerCachePath removeFile:self.fileName isTempFile:YES];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    [self.bkPlayerCachePath creatFileAtTempPath:self.tempPath];
    NSLog(@"Creat temp file path :%@" , self.tempPath);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (self.taskArr.count < 2) {
        ...
        //这里自己写需要保存数据的路径
        BOOL isSuccess = [self.bkPlayerCachePath copyTempVideoFileToIncompleteFileWithFileName:self.fileName];
        if (isSuccess) {
            NSLog(@"rename success");
        }else{
            NSLog(@"rename fail");
        }
        NSLog(@"----%@", [self.bkPlayerCachePath incompleterVideoWithName:self.fileName]);
    }
...
- (BKPlayerCachePath *)bkPlayerCachePath {
    
    if (!_bkPlayerCachePath) {
        _bkPlayerCachePath = [[BKPlayerCachePath alloc] init];
    }
    return _bkPlayerCachePath;
}

- (NSString *)tempPath {
    
    return [self.bkPlayerCachePath videoPathWithTemporaryFile:self.fileName];
}
